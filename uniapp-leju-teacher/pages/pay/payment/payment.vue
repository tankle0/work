		...
		<view class="block">
			<view class="content">
				<view class="orderinfo">
					<view class="row">
						<view class="nominal">订单编号:</view><view class="text">{{orderId}}</view>
					</view>
					<view class="row">
						<view class="nominal">订单金额:</view><view class="text">{{price/100}}元</view>
					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			this.amount = NP.round(e.price,2);
				...
				orderApi.payConfirm({
					...
					if(res.code == 'S'){
						setTimeout(()=>{
							...
							setTimeout(()=>{
								uni.redirectTo({
									url:'../../pay/success/success?price='+this.price
								});
							},300);
						},700)
					}else{
